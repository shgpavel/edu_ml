...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--clang", default="clang", help="Path to the clang executable.")
    parser.add_argument("--runs", type=int, default=3, help="Number of repetitions for each benchmark.")
    parser.add_argument("flags", nargs=argparse.REMAINDER, help="Clang compiler flags (prefix with --).")
    args = parser.parse_args()
    args.flags = [flag for flag in args.flags if flag != "--"]
    return args

def build_project(paths: Dict[str, Path], clang_path: str, clang_flags: List[str]) -> List[Path]:
    """Собирает libsodium и бенчмарки."""
    ...
    # --- ИСПРАВЛЕНИЕ ЗДЕСЬ ---
    # Принудительно запускаем autogen.sh для генерации 'configure'
    autogen_script = project_src / "autogen.sh"
    if autogen_script.exists():
        print(f"[*] Generating configure script via autogen.sh...")
        run_command(["./autogen.sh"], cwd=project_src)
    else:
        print("[WARN] autogen.sh not found, assuming 'configure' script already exists.")
    ...
    configure_cmd = [
        ...
        "--enable-static",      # Собираем статическую библиотеку
        "--disable-shared",     # Отключаем динамическую
    ]
    run_command(configure_cmd, cwd=build_dir, env=env)
    
    print(f"[*] Building and installing {PROJECT_NAME}...")
    run_command(["make", "-j", str(mp.cpu_count()), "clean"], cwd=build_dir)
    ...
    for bench_file in sorted(paths["bench_src"].glob("*_bench.c")):
        ...
        cmd = [
            clang_path, *clang_flags, f"-I{include_path}",
            str(bench_file),
            str(lib_path), # Статически линкуем
            "-o", str(exe_path),
            "-lpthread" # libsodium может требовать
        ]
        run_command(cmd)
...
def run_benchmarks(executables: List[Path], runs: int) -> List[Dict[str, Any]]:
    ...
    for exe_path in executables:
        ...
        try:
            ...
            results.append({"bench": exe_path.name, "bytes": binary_size, "seconds": avg_time})
            print(f"  - {exe_path.name:<25} {binary_size:8d} bytes, {avg_time:.6f}s avg")
